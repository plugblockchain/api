...
import fs from 'fs';
import { isString, stringCamelCase, stringUpperFirst } from '@plugnet/util';
...
interface TypeImports {
  codecTypes: TypeExist; // `import {} from '@plugnet/types/codec`
  localTypes: LocalExist;
  ownTypes: string[];
  primitiveTypes: TypeExist; // `import {} from '@plugnet/types/primitive`
  typesTypes: TypeExist; // `import {} from '@plugnet/types/types`
}
...
function generateRpcTypes (): void {
  ...
  const header = createImportCode(HEADER, [
    ...
    ...Object.keys(imports.localTypes).map((moduleName): { file: string; types: string[] } => ({
      file: `@plugnet/types/interfaces/${moduleName}`,
      types: Object.keys(imports.localTypes[moduleName])
    })),
    {
